...
def init_app(
    ...
    template = pn.template.MaterialTemplate(site="mpMRI viewer")
    ...
    page_slider = pn.widgets.IntSlider(
        name="Volumes",
        start=0,
    ...
    * To filter by the presence of a key, use `has(key)`
    ...
    * For float equivalences:
    ...
    def update_from_filter(event):
        ...
        previous_page_end = page_slider.end
        previous_page_value = page_slider.value
        try:
            ...
            page_slider.end = len(image_loader.dataset) - sqrt_n_images.value**2
            page_slider.value = 0
            page_slider.param.trigger("value_throttled")
            template.notifications.success(
            ...
            page_slider.end = previous_page_end
            page_slider.value = previous_page_value
            template.notifications.error(
    ...
    image = pn.pane.Image(
        pn.bind(
            ...
            start_idx=page_slider.param.value_throttled,
        ...
        pn.Column(
            ...
            pn.Column(
                pn.bind(
                    ...
                    sqrt_n_images.param.value_throttled,
                    page_slider.param.value_throttled,
                ),
                height=200,
            ),
    ...
    pn.extension(raw_css=[stylesheet])

    return template.main.append(pn.Column(image, page_slider).servable())
